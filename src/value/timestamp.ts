...
export type MockTimestampValue<State extends prismic.FieldState = prismic.FieldState> =
...
>(
	...
	if (config.state === "empty") {
		...
		const after = config.after ? new Date(config.after.toISOString().split("T")[0]) : undefined
		const before = config.before ? new Date(config.before.toISOString().split("T")[0]) : undefined
